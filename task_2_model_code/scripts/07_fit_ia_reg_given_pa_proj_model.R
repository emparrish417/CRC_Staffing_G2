...
# fit the model
ia_reg_given_pa_proj_model <- quap(
    
    alist(
        
        ia_registrations ~ dlnorm(meanlog = eta, sdlog = exp(log_sdlog)),
        
        eta <- alpha + mu[month] + rho[region] + beta * log(1 + pa_projects),
...
# write to file
save(ia_reg_given_pa_proj_model, file = 'models/ia_reg_given_pa_proj_model.RData')
