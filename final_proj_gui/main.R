...
pa_project_demand <- function(id, extreme) {
  ...
  #qplot(FAdist::rweibull3(1000, thres = total_lag_params$location, shape = total_lag_params$shape, scale = total_lag_params$scale))
  ...
  new_pa_projects_2 <- map_df(1:nrow(new_pa_projects), function(i) {
      ...
      tibble(project_id = new_pa_projects$new_project_id[i],
             disaster_date = new_pa_projects$date[i],
  ...
  #write_csv(all_pa_projects, 'all_pa_projects_50.csv')
